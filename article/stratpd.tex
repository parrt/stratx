...
\cut{In light of these challenges with marginal plots, linear regression, varying-coefficient models, and PD/ICE, we propose a stratification-based method that addresses effect heterogeneities, variable codependencies, and can be used to asses partial dependence in high dimensional settings.}
...
This paper proposes a stratification approach to isolating the effect of $x_c$ on target $y$ and has shown a few \spd{} and \cspd{} plots to highlight their advantages over PD and ICE plots. In this section, we provide more examples on synthetic and real data sets and investigate the effect of noisy data\cut{, and examine how \spd{} deals with edge cases arising from unusual \xnc{} partitioning}.  All plots, including the PD/ICE plots, were generated using the Python {\tt stratx} library and script {\tt genfigs.py} (in the github repository) generated all figures in this paper. (PD/ICE plots were derived from random forest models with 100 trees and minimum samples per leaf of 1). 
...
Experience applying \spd{} suggests two limitations. First, \spd{} can only cancel out contributions from variables included in the $\bf X$ presented to it, but of course this is true for any partial dependence method. For example, \figref{fig:bulldozer} is likely biased towards a traditional marginal plot than is necessary, because we included only three codependent features in $\bf X$ for demonstration purposes.  
