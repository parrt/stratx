...
In this section, we demonstrate experimentally that \spd{} and \cspd{} compute accurate partial dependence curves for synthetic data and plausible results for real data sets.
...
\caption{\small $y = x_1^2 + x_1 x_2 + 5 x_1 sin(3 x_2) + 10$ where $x_1,x_2,x_3 \sim U(0,10)$ and $x_3$ does not affect $y$. No noise added.  $R^2$  0.9973}
...
Consider the synthetic weather data shown in \figref{fig:statetemp}(a) where temperature varies in sinusoidal fashion over the year and with different baseline temperatures per state:

\begin{equation}\label{eq:weather}
y = t[x_{\it state}] + 10 sin(\frac{2\pi}{365}x_{\it dayofyear}+\pi) + \epsilon, ~~~ \epsilon \sim N(0, 4)
\end{equation}

\noindent where the baseline $t$ per state is $\{{\it AZ}=90, {\it CA}=70, {\it CO}=40, {\it NV}=80, {\it WA}=60\}$. Each sinusoid in \figref{fig:statetemp}(a) is the average of three years' temperature data.
...
\caption{\small (a) Marginal plot of bulldozer {\tt YearMade} versus {\tt SalePrice} using subsample of 10k observations, (b) partial dependence drawn by SHAP interrogating an RF with 40 trees and explaining 1000 values with 100 observations as background data, (c) \spd{} partial dependence.}
\label{fig:yearmade}
\end{center}
\end{figure}
...
\caption{\small (a) Marginal plot of bulldozer {\tt YearMade} versus {\tt MachineHours} using subsample of 10k observations, (b) partial dependence drawn by SHAP interrogating an RF with 40 trees and explaining 1000 values with 100 observations as background data, (c) \spd{} partial dependence.}
\label{fig:yearmade}
\end{center}
\end{figure}
