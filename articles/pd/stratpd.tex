...
\usepackage{listings}
\usepackage{array}
\usepackage[noline, vlined]{algorithm2e}
\usepackage{lmodern}
\usepackage{caption}
...
\For{each leaf $L \in T$}{
        $({\bf x}_L, {\bf y}_L)$ := $\{(x_j^{(i)},  y^{(i)})\}_{i \in L}$\Comment*{\it Get leaf samples}
	${\bf ux}, \bar{\bf y}$ := Group and sort $({\bf x}_L, {\bf y}_L)$ by $x_j$ value, computing $\bar{y}$ per unique $x_j$ value\\
	${\bf dx}$ := ${\bf ux}^{(i+1)} - {\bf ux}^{(i)}_{i=1..|{\bf ux}|-1}$ for $i = 1..|{\bf ux}|-1$\Comment*{\it Discrete difference between adjacent}
...
\For{each leaf $l \in T$}{
        $({\bf x}_l, {\bf y}_l)$ = $\{(x_j^{(i)},  y^{(i)})\}_{i \in l}$\Comment*{\it Get leaf samples}
        ${\bf ux}$, ${\bf cx}$ := $unique({\bf x}_l)$\Comment*{\it Get unique categories, counts from leaf samples}
