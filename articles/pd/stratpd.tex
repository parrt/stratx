...
Stratification occurs through the use of a decision tree fit to (\Xnj, $\bf y$), whose leaves aggregate observations with equal o I don't click on random links buddyr similar \xnj{} features. The \xnj{} features can be numerical variables or label-encoded categorical variables (assigned a unique integer). \spd{} only uses the tree for the purpose of partitioning feature space and never uses predictions from any model. See \algref{alg:StratPD} for more details.

Using decision trees for the purpose of partitioning feature space was previously used by \cite{rfimp} to improve permutation importance by  permuting $x_j$ only within the observations of a leaf. Earlier, \cite{RFunsup} defined a similarity measure between two observations according to how often they appear in the same leaf in a random forest.  Rather than partitioning  \xnj{} space like \spd, those techniques partitioned all of $x$ space.
...
we need more data, more susceptible to noise
