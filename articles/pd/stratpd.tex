...
number of leaves is $n / {\tt\small min\_samples\_leaf}$. computing $y$ deltas across all leaves is roughly the same as computing...  the size is approximately {\tt\small min\_samples\_leaf} and there are... in the worst case, for non-integer values, stratification leads to leaf sizes about {\tt\small min\_samples\_leaf}. (best case, everything in \xnj is exactly equal and we end up with a single leaf). worst-case every $x_j$ value in the leaf is unique, which leads to multiple operations of size {\tt\small min\_samples\_leaf}. the second loop iterates over all possible unique values in ${\bf X}_j$ and each iteration costs {\tt\small min\_samples\_leaf} to extract slopes covering unique x values. then more passes over the unique x sized data sets followed by a cumulative sum at the cost of another |ux|.

The cost to compute $y$ deltas for all observations among the leaves is linear in $n$ and the cost to average slopes across leaves is on the order of $|unique({\bf X}_j)| \times n$ or $n^2$ when all ${\bf X}_j$ are unique in the worst case. \spd{} is, thus, $O(n^2)$ in the worst case.

\cspd{} computes category deltas linearly in $n$ but must make multiple passes of size $|T|$ (number of leaves), averaging in leaf category delta vectors. Averaging delta vectors costs $n$ operations, so each pass requires $|T| \times n$. Since the number of leaves is roughly $|T| \approx n / {\tt\small min\_samples\_leaf}$, each pass requires $n / {\tt\small min\_samples\_leaf} \times n = n^2 / {\tt\small min\_samples\_leaf}$ or $O(n^2)$. The number of passes depends on the data set
