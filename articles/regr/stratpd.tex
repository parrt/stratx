...
all plots are done using {\tt stratx} library, including the PD/ICE plots.  PD/ICE use random forests with 100 trees and minimum samples per leaf of 1. \todo{James: can you check the ICE paper when they describe that the interactions of $x_3$ are just around 0?  From the equation they are clearly $\ge 0$.}

We begin by reproducing graphs from \cite{ICE} big X from ICE, additivity from PD/ICE. $y = 0.2x_1 - 5x_2 + 10x_2\mathbf{1}_{x_3 \geq 0} + \epsilon$, $x_1, x_2, x_3 \in U(-1,1)$, Sample size 1000.  they used stochastic gradient boosting model (SGB)   for additivity, they used a general additive model with $y = x_1^2 + x_2 + \epsilon$ with $x_1, x_2 \sim U(-1,1)$, $\epsilon \sim N(0,1)$.

everyone was okay but codependent features cause problems as we saw in \figref{fig:height_vs_weight}; here's another; show education versus weight \figref{fig:education_vs_weight} The data set has an equal number of males and females with $x_{pregnant} \sim U(0,1)$ if female, 65 + $U(-4.5,5)$ if female, and $x_{height} = 68 + U(-7,8)$ if male. 

\[
weight = 120 + 10(x_{height} - min(x_{height})) + 30x_{pregnant} - 1.2x_{education}
\]

 As another example, compare the PD/ICE (c) and \spd{} (d) plots in \figref{fig:education_vs_weight} showing number of years of education versus weight using the same data set. Weight is related to education by slope -1.5 so a perfect partial dependence graph would show a drop of 12 pounds over 10 years of education.   The PD/ICE plot captures only about half of that relationship, whereas, the \spd{} plot gets much closer to the true education-weight relationship. Female observations have at least 12 years of education, versus 10 for males, so $x_{education}$ and $x_{sex}$ are codependent, though, there is no interaction term. The fact that women are shorter on average biases the education-weight PD/ICE plot because the baseline weight is lower from which the education contribution is  subtracted.
...
\subsection{Compensating for noisy features}
...
\subsection{Pathological \xnc{} and $x_c$ partitioning issues}
