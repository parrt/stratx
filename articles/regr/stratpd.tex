...
To illustrate how variable dependencies result in misleading PD and ICE plots, consider a  weight data set with observations matching a person's characteristics to a weight in pounds. We discuss the data set details in \secref{sec:exp}, but for the moment, assume that women are slightly shorter on average and are 30 pounds heavier if pregnant. The PD/ICE plot in \figref{fig:height_vs_weight}(a) shows an inaccurate partial dependence where shorter people are slightly heavier than those over about 72 inches. The blue ICE lines pop up significantly for shorter heights due to the codependence of $x_{sex}$ and $x_{pregnant}$: PD and ICE conjure up pregnant males and ask the model to estimate their weight. In contrast, the \spd{} plot in \figref{fig:height_vs_weight}(b) is not confused by codependence and gives the true partial dependence of weight on height.
...
\subsection{\spd{} for numeric data}
...
\spd{} trains a random forest in the usual way but on $({\bf X}_{(-c)}, {\bf y})$ rather than $({\bf X}, {\bf y})$, where ${\bf X}_{(-c)}$ is the explanatory matrix {\em without} column vector $x_c$, $\{(x_1, .., x_{j \neq c}, .., x_p)\}$.  Observations $({\bf x}_{i}, y_i)$ that end up in a tree leaf are more likely than not to be similar, but similar only in columns $x_{(-c)}$ since the forest trained on ${\bf X}_{(-c)}$. (\todo{cite breiman similarity mechanism}.)   Training must ensure individual decision trees in the forest have at least two samples per leaf in order to fit localized linear models. With too few observations in leaf $L$, $\beta_L$ is a poor estimate; with too many observations, $\beta_L$ is overly-sensitive to contributions from non-$x_c$ features. \spd{} degenerates to a simple marginal plot if training yields a single tree with a single node containing all of $\bf X_{(-c)}$.  Algorithm \ref{alg:StratPD} encodes the \spd{} process and the full Python 3 source code is available at {\small \url{https://github.com/parrt/stratpd}}.
...
Before moving on to sample applications of \spd{}, it's worthwhile emphasizing the advantages of this approach over PD and ICE. Most importantly, \spd{} is model-independent in the sense that it does not expect nor rely on a user's model trained on $({\bf X}, {\bf y})$.  \spd{} does, of course, use models internally: a random forest for stratification and multiple linear models for estimating local partial derivatives.  The biggest difference is that \spd{} does not use predictions from any these models, which means prediction accuracy is irrelevant. unlike for PD and ICE. (\todo{refer to unsupervised learning trick}.)

\spd{} was designed to support non-independent features, which violates an assumption of PD and ICE. While ICE exposes variable interactions, it is still susceptible to variable dependencies causing misleading plots.
...
\includegraphics[scale=0.7]{images/height_vs_weight_stratpd.png}
\includegraphics[scale=0.7]{images/height_vs_weight_pdp.png}
\caption{{\bf  Edu and height vs weight}}
\label{fig:edu_height_vs_weight}
\end{center}
\end{figure}
