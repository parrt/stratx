...
Train random forest regressor {\it rf} on ${\bf X}_{(-c)}$, $\bf y$\\
Let $D$ be dictionary mapping $cat$ to $y$\\
\ForEach{leaf $L \in {\it rf}$}{
	$(L_x, L_y)$ = $\{(x_{ic},  y_i)\}_{i \in L}$\\
	\ForEach{$cat \in L_x$}{
		ycats = $\Sigma_{i \in L_x = cat} L_y^{(i)}$ average $L_y$ per category $cat$\\
	}
	Let dictionary $y_{cat}$ = average $L_y$ per category $cat$\\
	\lIf{$|y_{cat}| < 2$}{{\bf continue}}\\
	\lForEach{$cat \in y_{cat}$}{$y_{cat}$ = $y_{cat}$ - $min(L_y)$}
}
\Return{collection of all $\beta_R$ and $\hat{y}$}
\end{algorithm}
