...
%		Add $L'$ to piecewise list\\
		...
		\lIf{left$(R_{L'})$ = right$(R_{L'})$}{{\bf continue}}\\
...
\SetAlgoSkip{}
\SetInd{.5em}{.5em}
\TitleOfAlgo{{\em CatStratPD}($X$, $\bf y$, $c$, $refcat$) {\bf returns} dictionary mapping category to effect on $y$}
Let $D$ be dictionary mapping category to $y$ value\\
Train decision tree regressor $T$ on (\xnc{}, $\bf y$)\\
\ForEach{leaf $L \in T$}{
	$(L_x, L_y)$ = $\{(x_{ic},  y_i)\}_{i \in L}$\\
	Let $avg$ be dictionary mapping category to average leaf $y$ value\\
	cats = unique cats  $\in L_x$\\
	\ForEach{$cat \in cats$}{
		$cat_y$ = $L_y[L_x=cat]$\tcp*[r]{\it Group leaf $L_x$ by $cat$, get $y$'s}
		\lIf{$|cats| < 2$}{{\bf continue}}\\
		$avg[cat]$ = $mean(cat_y)$\\
	}
	\ForEach{unique $cat \in L_x$}{
%		$avg[cat]$ = $avg[cat] - mean(L_y[refcat])$\tcp*[r]{\it Strip y contribution from \xnc{}}
		$D_{cat}$ = $D_{cat} + (avg[cat] - avg[refcat])$\tcp*[]{\it Strip y contribution from \xnc{}}
	}
}
