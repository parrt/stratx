...
When all or most of the $x_c$ values are discrete integers, some choices of $nbins$ lead to virtually all bins in all leaves getting weight 0. Also, binning integers leads to awkward bins, such as 1.8 to 3.2 bedrooms or dayofweek 1.3 to 2.1. In the discrete case, hyper parameter $nbins$ is ignored and leaf bins are defined as the ordered adjacent value pairs from unique$(x^{(L)})$; e.g., for unique $x^{(L)}$ values $(1,3,4,5)$, the bin ranges are $((1,3), (3,4), (4,5))$. The rest of Algorithm 1 remains the same.
...
\todo{compare continuous vs discrete; show ignored}
