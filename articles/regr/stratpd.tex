...
\SetAlgoSkip{}
\SetInd{.5em}{.5em}
\TitleOfAlgo{{\em StratPD}($X$, $\bf y$, $c$, $hires$=30) {\bf returns} collection of $\beta$ coefficients, $pd$}
Train decision tree regressor $T$ on (\xnc{}, $\bf y$)\\
%$leaves$ = leaves($T$)\\
%piecewise = [] \tcp*[r]{List tracking piecewise leaves from $T'$}
\ForEach{leaf $L \in$ leaves$(T)$}{
	$(L_x, L_y)$ = $\{(x_{ic},  y_i)\}_{i \in L}$\\
	Train decision tree regressor $T'$ on $(L_x, L_y)$\\
	\ForEach{leaf $L' \in$ leaves$(T')$}{
%		Add $L'$ to piecewise list\\
...
\SetAlgoSkip{}
\SetInd{.5em}{.5em}
\TitleOfAlgo{{\em CatStratPD}($X$, $\bf y$, $c$, {\it refcat}) {\bf returns} mapping from cat to effect on $y$}
