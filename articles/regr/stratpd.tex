...
The stratification approach can also capture how a categorical variable $x_c$ affects $y$, instead of just a single category at a time (if one were forced to dummy-encode $x_c$). But, because categorical variables can be unordered nominal variables, the notion of $y$ slope is not meaningful between two categories. Instead, the \cspd{} algorithm groups leaf observations $(L_x, L_y)$ by the categories in $L_x$, computes the average $L_y$ value per category, and subtracts the mean of $L_y$ for a reference category from all category averages to erase the $y$-contributions of \xnc{}. To get the overall contribution, then, of $x_c$ on $y$ for category $cat$, \cspd{} averages the leaf contributions for $cat$ from all $L'$ weighted by $|L|$. See Algorithm \ref{alg:CatStratPD}.
