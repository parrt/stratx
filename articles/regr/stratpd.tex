...
\ForEach{tree $T \in \it rf$}{
    \ForEach{leaf $L \in T$}{
        ...
        Let $n_x=|unique(x^{(L)})| \le |unique(X_c)|$\\
%        Let $K=\{k_1, k_2, \,\dots, k_{n_x}\}$ where \\
        $n^{(L)} = \begin{cases} |L| &\mbox{if } n_x > 1 \\ 
...
$n = \Sigma_{T \in {\it rf}} \Sigma_{L \in T} ~n^{(L)}$~~~~~~~~~~~~~~~~~~~~~~({\it Num supporting observations})\\
$\Delta^{(k)} = \frac{1}{n} \Sigma_{L} n^{(L)}\Delta^{(L,k)}$~~~~~~~~~~~~~~~~~~~({\it Change for $k$ is weighted average across leaves})\\
