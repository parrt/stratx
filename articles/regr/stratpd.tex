...
\subsection{Correlated, dup'd}\label{sec:dup}
...
\SetAlgoSkip{}
\SetInd{.5em}{.5em}
\TitleOfAlgo{{\em StratPD}($X$, $\bf y$, $c$, $\it ntrees=1$, $\it bootstrap=false$, $\it split\_features=all$)}
\KwOut{collection of $\beta$ coefficients, partial dependence curve}
Train random forest regressor $\it rf$ on (\xnc{}, $\bf y$) with $\it ntrees$, $\it bootstrap$, $\it split\_features$ \\
\ForEach{tree $T \in \it rf$}{
    \ForEach{leaf $L \in T$}{
    	$(L_x, L_y)$ = $\{(x_{ic},  y_i)\}_{i \in L}$\\
    	Train decision tree regressor $T'$ on $(L_x, L_y)$\\
    	\ForEach{leaf $L' \in T'$}{
...
\SetAlgoSkip{}
\SetInd{.5em}{.5em}
\TitleOfAlgo{{\em CatStratPD}($X$, $\bf y$, $c$, $\it ntrees=1$, $\it bootstrap=false$, $\it split\_features=all$)}
\KwOut{Mapping from cat to effect on $y$}
...
$n = \Sigma_{L \in T}S[L]$\tcp*[r]{\it Num observations used to compute averages}
$D[cat] = \frac{1}{n} D[cat]$\tcp*[r]{\it Get average of leaf averages for $cat$}
$D[cat] = D[cat] - \underset{cat}{min}(D[cat])$\tcp*[r]{\it Cat with lowest $\overline{y}_{cat}$ is now 0}
\Return{$D$}
\end{algorithm}
