...
The key idea is to relax stratification so that it organizes observations into groups of similar rather than equal observations.  Our approach, called \spd, uses a random forest to cluster observations into  groups of similar observations and then collects piecewise approximations to the partial derivatives of $f(\bf x)$ for the groups. The approximation to the partial derivative for range $R$ of $x_c$ is the average of regression coefficients that  overlap with $R$.  The random forest trains on $({\bf X}_{(-c)}, {\bf y})$, in the usual way, where ${\bf X}_{(-c)}$ = $\{(x_1, .., x_{i \neq c}, .., x_p)\}_{i=1}^N$ is  the explanatory matrix {\em without} column vector $x_c$, ${\bf X}_{(-c)}$.  Observations $({\bf x}_{i}, y_i)$ that end up in a tree leaf are more likely than not to be similar in $x_{j \neq c}$ since the forest trained on ${\bf X}_{(-c)}$. (\todo{cite breiman similarity mechanism})
