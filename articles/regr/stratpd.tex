...
Let $D$ be dictionary mapping leaf, category to $y$ value\\
Let $S$ be dictionary mapping leaf to leaf size\\
Train decision tree regressor $T$ on (\xnc{}, $\bf y$)\\
\ForEach{leaf $L \in T$}{
	$(L_x, L_y)$ = $\{(x_{ic},  y_i)\}_{i \in L}$\\
	$y_{cat}$ = $L_y[L_x=cat]$\tcp*[r]{\it Group leaf $L_x$ by $cat$, get $y$'s}
	$\Delta_y[cat] = mean(y_{cat}) - mean(y_{\it refcat})$\tcp*[r]{\it Strip \xnc{}'s y contribution}
	$S[L]$ = $\begin{cases} |L| &\mbox{if } |{\it unique~cats \in L_x}| > 1 \\ 
0 & otherwise\end{cases}$\\
	$D[L,cat] = D[L,cat] + \Delta_y[cat] \times S[L]$\tcp*[r]{\it Weight leaf average for $cat$ by leaf size}
}
%Let $n_{cat}$ be number of $avg[cat]$ values added to $D_{cat}$\\
$n = \Sigma_{L \in T}S[L]$\tcp*[r]{\it Num observations used to compute averages}
$D[L,cat] = D[L,cat] / n$\tcp*[r]{\it Get average of leaf averages for $cat$}
%$ = $\frac{1}{\Sigma_{L \in T}|S[L]|} \Sigma_{i=1}^{|S_{cat}|} D_{cat}$\tcp*[r]{\it Weighted average of leaf averages for $cat$}
\Return{$D$}
\end{algorithm}
