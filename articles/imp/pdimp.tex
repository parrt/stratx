...
Speed is an important consideration when using feature importance techniques for feature selection---practitioners need to iterate quickly when developing models.  \simp{} is model-free and, therefore, does not incur the cost of training a model, except for the single decision tree used for stratification purposes to get partial dependences.   The cost of computing each $x_j$'s impact is the cost of computing the partial dependence and a pass over the unique $x_j$ values to compute the mean. To compute importance, it costs an extra pass over all $n$ $x_j$ values to get the histogram. The upper bound performance for \spd{} and \cspd{} partial dependences is $O(n \times |unique(x_j)|)$, or just $O(n^2)$ in the worst case.   Because $|unique(x_j)|$ is usually much smaller than $n$ in real data sets, \spd{} typically performs linearly.  The overall worst case behavior for \simp{} to get $p$ impacts is then $(p + |unique(x_j)|)\times n^2$ and $(p + |unique(x_j)|+n)\times n^2$ for importances, giving asymptotic behavior $O(n^2)$ and $O(n^3)$. 
...
\hline
{\tt\small flight} & 17 & 5 & 0.6s & 17.7s & 64.6s & 0 &
{\small $-0.088 n + 0.164 n^2$} & {\small 0.9995}\\
{\tt\small bulldozer} & 14 & 2 & 0.2s & 2.7s & 8.7s & 0 &
{\small $0.101 n + 0.016 n^2$} & {\small 0.9998}\\
{\tt\small rent} & 20 & 0 & 0.3s & 3.7s & 8.1s & 0 &
{\small $0.367 n + 0.003 n^2$} & {\small 0.9994}\\
\end{tabular}
...
In practice, importances for numerical variables are nearly linear and importances for categorical variables are quadratic. \figref{fig:timing} summarizes the empirical time in seconds to compute importances for subsets of the three Kaggle data sets. The ``time versus $n$'' and $R^2$ columns describe a quadratic fit to the time required to compute subsets from $n=1$ to 30,000 stepping by 1,000. The rent data set has no categorical variables and grows linearly with $n$, possibly with a small quadratic component. The flight data set, on the other hand, shows clear quadratic behavior due to the (five) categorical variables.

Despite the highly nonlinear worst-case complexity, \figref{fig:timing} suggests our Python-only \simp{} prototype is fast enough for use on real data sets of sizes in the tens of thousands. Our casual attempts at parallelizing the computation of $p$ importances across $p$ CPU cores were thwarted by Python's dreaded ``global interpreter lock'' (threading) and passing data between processes (process-based threading).  We did, however, get increased performance  from using the Numba just-in-time compiler ({\tt\small http://numba.pydata.org}) on algorithm hotspots. Reimplementation in C would be the next step and should yield dramatic performance improvements over pure Python.
