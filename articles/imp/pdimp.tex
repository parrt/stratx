...
Next, we compare \simp{} to OLS, OLS-based SHAP, RF-based SHAP, and RF-based permutation importance in \figref{fig:topk}.  We have given SHAP every advantage here.  Except for OLS-based SHAP, SHAP uses an RF fit to the training data to compute feature importances and then trains the same kind of model (RF) to measure top-$k$ error curves.  Further, SHAP selects features using the validation data, the very data used to compute error curves. In contrast, \simp{} cannot use predictions from a fitted model, by design, and we restrict \simp{} to examining just the training data, without access to the validation data. Despite these handicaps, \simp{} error curves for the four data sets are competitive with the  feature rankings from the other methods, as shown in \figref{fig:topk}. 

\simp{} performs competitively with existing techniques, sometimes a little worse and sometimes a bit better than RF-based SHAP and RF-based permutation importance, but \simp{} does so without the expertise of a fitted model and with access to less data.  In fact, the first feature recommended by \simp{} for the bulldozer data set gets half the error rate  of the top features recommended by the other methods, \figref{fig:topk-impact}(c). \simp{} recommends the {\tt ModelID} followed by {\tt YearMade}, whereas RF-based SHAP selects {\tt ProductSize}, {\tt YearMade}, followed by {\tt ModelID}. \todo{tie into the importance bar charts}


We define feature importance as the area under the partial dependence curve weighted by the $x_j$ density, which takes into consideration validation sets with highly skewed distributions.  That said, we have found that the plain impact, area under the partial dependence curve, also tends to work well, as shown in \figref{fig:topk-impact}. In some cases, importances derived from \simp{} outperform impact-derived importances, as they do initially for the flight data set; compare \figref{fig:topk}(b) and \figref{fig:topk-impact}(b).
...
OLS does not appear to adequately capture the relationship between features and response variable, except for the Boston data set, which explains why the OLS-based SHAP error curves are higher than for the other methods. (Curiously, features ranked by OLS SHAP perform worse than plain OLS.) 
