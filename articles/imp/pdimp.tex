...
\title{\bf Tech Report:\\
Feature Impact and Importance without model predictions}
...
\todo{For RFs...gini drop computed as part of construction \cite{CART} and \cite{PDP}; biased: \cite{permbias} \cite{RFunbiased}} very similar to \simp{} in that it uses a tree model but no predictions from the model; it derives values from the model.
...
The current Python prototype is fast enough to analyze data subsets in the tens of thousands of records. (It has a worst-case quadratic complexity but exhibits linear behavior for numerical and slightly quadratic behavior for categorical features; see experimental results). \todo{p times quadratic actually}
\todo{Costs} FPD is $O(nm)$ for $n$ records and $m$ curve evaluation points and feature codependence biases the curves. ALE is very efficient ($O(n)$ per \citealt{ALE}).  SHAP has the hardest time with efficiency due to the combinatorial problem of subsetting features and, in theory, isolates $x_j$'s contribution in the presence of codependence. Empirically, however, we find that codependent features can bias SHAP's partial dependence curves \todo{as discussed where?}. To measure impact as area under the curve rather than the area straddling the mean, the curves of FPD, ALE, and SHAP must be shifted so that the left edge of $x_j$'s range starts with a partial dependence of 0. Then, the average curve magnitude would estimate the ideal impact. 
\todo{Our final concern is that SHAP effectively limits the user's choice of model to those with model-type-dependent implementations, such as {\tt\small TreeExplainer}, for efficiency reasons. For example, SHAP applied to a support vector machine (without a model-specific optimization) trained on Boston's 506 records takes 13 minutes to explain the same 506 records.  Large data sets also pose problems even for the model-type-dependent optimizations. SHAP applied to a modest 50-tree RF trained on the 1.2M record ``League of legends'' data set \citep{lol} takes 25 minutes to explain a sample of size 1000.

SHAP has model-type-dependent optimizations for linear regression, deep learning, and decision-tree based models.
}
