...
\caption{\small $y = x_1^2 + x_1 x_2 + 20*sin(3 x_2) + 10$ with $x_j \sim U(0,10)$ where $x_3$ has no effect on $y$ and $n=1000$}
\label{fig:shap-weight}
\end{center}
\end{figure}

SHAP and \simp{} appear dissimilar on the surface, but they share the same overall strategy: Isolate the effect of $x_j$ upon $y$, even in the presence of highly-codependent features, then measure the average $x_j$ deviation (at several $x_j=z$ locations) from a common reference point. The techniques differ only in how they isolate $x_j$ from the effects of other features and in the common reference point. SHAP isolates $x_j$ by tracking $x_j$'s $\hat{f}(x)$ marginal contribution to all possible feature subsets, whereas \simp{} stratifies and examines the data set directly (via \spd{} and \cspd). SHAP measures deviations from $\bar{y}$ to obtain the area straddling $\bar{y}$ and \simp{} measures deviations from 0 to obtain the area under the PD curve.
...
Second, while the formal SHAP definition does not assume feature independence, the SHAP implementation does in order to simulate models trained on feature subsets. The SHAP definition approximates $\hat{f}_S(x_S)$ with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}}) | {\bf X}_S = x_S]$, but SHAP's implementation approximates $\hat{f}_S(x_S)$ with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$, which requires feature independence for accuracy. \cite{janzing2019feature} argues that ``{\em unconditional} [as implemented] rather than {\em conditional} [as defined] expectations provide the right notion of dropping features.''

In practice, however, we have observed several instances of questionable SHAP partial dependence curves for features important to the model.  \figref{fig:shap-stratpd-YearMade} gives one such example, comparing the marginal plot of  bulldozer {\tt YearMade} to {\tt SalePrice} (a), SHAP partial dependence of SalePrice on YearMade (b), and \spd{}'s curve (c). The SHAP plot does not show the expected relationship, that of decreased value for older bulldozers. One reasonable possibility is that other features are bleeding in, biasing the effect of {\tt YearMade}.
...
\noindent with 50/50 male/female, half of the women are pregnant, and women gain an exaggerated 70 pounds to make the bias more prominent. \figref{fig:shap-weight}(a) illustrates the SHAP ``tree path dependent'' partial dependence between {\tt height} and {\tt weight} and (b) shows that the interventional SHAP curve is better but still biased by codependent features.  The exact partial dependence relationship between {\tt height} and {\tt weight} should be a line with slope 10, which is what \spd{} computes and draws on top of the SHAP plots (using righthand scale) in \figref{fig:shap-weight}; see \citet{stratpd} for more details.

The bias arises because of the marginal expectation $FPD_S(x) = \Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ (Equation 53 in \citealt{PDP} gives the equivalent summation).  Computing $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ effectively replaces the $S$ features in all $\bf X$ records with $x_S$, which potentially conjures up nonsensical records, or at least records not in $\hat{f}$'s training set.  This introduces bias into each feature subset's estimate of $x_j$'s marginal effect on $y$, and it is unclear that the biases average out.

Our final concern is that SHAP effectively limits the user's choice of model to those with model-type-dependent implementations, such as {\tt\small TreeExplainer}, for efficiency reasons. For example, SHAP applied to a support vector machine (without a mode-specific optimization) trained on Boston's 506 records takes 13 minutes to explain the same 506 records.  Large data sets also pose problems even for the model-type-dependent optimizations. SHAP applied to a modest 50-tree random forest trained on the 1.2M record ``League of legends'' data set \citep{lol} takes 25 minutes to process a sample of size 1000.

 SHAP goes to great lengths to isolate features using subsets then biases the results by assuming independence for efficiency reasons.  


** partial dependence curve seems off which led us to check...

** permutation biases results

** speed limits choice

** if it is biased may be FPD is good enough. see graphs

, then SHAP's implementation further approximates with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ by assuming feature independence.   SHAP engages this behavior in ``interventional'' mode, and users can pass in a so-called {\em background set} that contains, for example, $x_j$ averages or even the entire $\bf X$ training set.

SHAP's implementation assumes feature independence for the purpose of simulating models with missing features, but still attempts to isolate $x_j$'s effect on $y$ by measuring the average effect of introducing $x_j$ to all possible features subsets.  Assuming feature independence throughout would make checking feature subsets unnecessary. In fact, 


A SHAP implementation that never assumed independence would correctly isolate $x_j$'s importance and, thus, have a clear advantage over the FPD approach, which is biased in the presence of codependencies. The PD partial dependence defined above, on the other hand, isolates $x_j$ even in the presence of codependent features. That means that SHAP importance is equivalent to the average magnitude of a mean-centered PD curve. (Note that \simp{} measures impact as area under the PD curve rather than the area straddling the mean-centered PD curve.) SHAP and \simp{}, therefore, both measure importance as a function of hopefully-unbiased partial dependence curves, SHAP using feature subset permutations to isolate $x_j$ effects and \simp{} using data stratification. This neatly fits SHAP into the framework described in this paper.
...
\todo{is $\Ex [\hat{f}(x) | {\bf X}_j = z ]$ actually $FPD_j(x)$, which is $\Ex [\hat{f}(x_j, {\bf X}_{\bar{j}} ]$?}
