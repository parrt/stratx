...
default is one trial
boston: min samples per x = 1
rent: defaults
...
Using partial derivatives to isolate the effect of variables on the response was used prior to \spd{} by ALE and {\em Integrated Gradients} (IG) from \cite{intgrad}. The key distinction is that  \spd{} integrates over the derivative of the generator function, ${\partial f}/{\partial x_j}$, estimated from the raw training data, whereas, previous techniques integrated over the derivative of model $\hat{f}$ that estimates $f$: ${\partial \hat{f}}/{\partial x_j}$. While there are advantages to using models, such as their ability to smooth over noise, properly choosing and tuning a machine learning model presents a barrier to large user communities, such as business analysts, scientists, and medical researchers. Further, without potential distortions from a model, \spd{} supports the measurement of impact, not just importance.
...
\noindent This formula works for any partial dependence curve, which must be computed by examining output from a fitted model, $\hat{f}$, or by estimating the partial derivative of $f$ directly from the data and then integrating, as \spd{} does. Dividing impact by the sum of all impacts, converts impact units from $y$'s units to [0..1]:
...
\noindent Let $P(x_j)$ be the probability of seeing $x_j$ in ${\bf X}_j$, then:
...
These definitions of \Imp{} and \Impo{} also apply to categorical explanatory features encoded as unique integers because the mean operation is insensitive to element order; this avoids any assumption about order for nominal categoricals.  For example, consider categorical feature $x_{state}$ and numerical feature $x_{bedrooms}$ from an apartment rent data set.  The non-normalized \Imp{} score for both is just the average magnitude of the associated partial dependence curve, using \spd{} for $x_{bedrooms}$  and \cspd{} for $x_{state}$ (the category equivalent of \spd{} from the {\tt stratx} package).  If, on average, each of 50 US states affected rent price by \$200 and, on average each of five discrete bedroom counts affected rent price by \$200, then $x_{bedrooms}$  and $x_{state}$ would have the same impact.
...
Features that are predictive in one model are not necessarily predictive in another model.  To determine how well the feature rankings from the various methods ``export'' to other models, we trained gradient boosting machines (GBM) on the OLS-based, RF-based, and \Impo{} feature rankings.  \figref{fig:topk-gbm} shows that the \Impo{} error curves generated using GBM models are similar to those resulting from RF models, except for the flight delay curve, which does not improve much after feature $k=5$, while the other feature rankings do improve. 
...
To assess the quality of \simp's feature impacts, we compared \simp's recommended feature importance rankings to those of other techniques, as a proxy. Despite not having access to predictions from a fitted model, \simp{} feature rankings are competitive with other techniques on Boston and three real data sets.  \figref{fig:topk}c illustrates a case where \Impo's most important feature yields half the error rate of the top features recommended by the other methods.  One would expect model-based techniques to easily identify the single most important model feature, or at least more readily than an approach not using model predictions.  Even if such results are rare, misidentifying this top feature indicates that there is room for improvement in the feature importance research area. 
