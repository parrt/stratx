...
\todo{yikes, This is getting too big and must be split apart or subsectioned}
...
SHAP's implementation assumes feature independence for the purpose of simulating models with missing features, but still attempts to isolate $x_j$'s effect on $y$ by measuring the average effect of introducing $x_j$ to all possible features subsets.  Assuming feature independence throughout would make checking feature subsets unnecessary. In fact, assuming independence, there is a simple relationship between SHAP and mean-centered FPDs, the partial dependence curves as originally defined by Friedman: the average magnitude (importance) of SHAP values and mean-centered FPD curves coincide.  We state the following theorem and prove it in the appendix.
...
, then SHAP's implementation further approximates with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ by assuming feature independence. \cite{janzing2019feature} argues that ``{\em unconditional} [emphasis theirs] rather than {\em conditional} expectations provide the right notion of dropping features.''  SHAP engages this behavior in ``interventional'' mode, and users can pass in a so-called {\em background set} that contains, for example, $x_j$ averages or even the entire $\bf X$ training set.
...
\figref{fig:shap-weight}, \figref{fig:FPD_vs_SHAP}
