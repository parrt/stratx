...
\[
\phi_j(\hat{f},x) = \text{\it PDP}_{F}(x) - \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x)
\]

\noindent  since $S = F \texttt{\char`\\} \{j\}$ and the $1/p$ weight is common to all features and can be dropped as it does not affect the relative impact of the features.
