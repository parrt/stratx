...
While SHAP has very strong theoretical foundations, we cannot agree with the SHAP definition of feature impact for the simple reason that SHAP measures impact as the magnitude of deviations a feature causes from the mean model prediction, $\overline{y}$, rather than deviations from zero.   For example, given equation $y=x_1^2 + x_2 + 100$ from \figref{fig:combined-area}, the effect of $x_1$ on $y$ is clearly $x_1^2$ (impact is area of orange region), not $|\overline{y} - x_1^2|$, as defined by SHAP (area of grey region).  \todo{need to explain why they are wrong or is it obvious?} 
...
Our first experiment is to compare the feature rankings recommended by \simp{} to PCA (``loads'' associated with the first component) and Spearman's R coefficient computed between each $x_j$ and the response variable, $y$. \figref{fig:baseline} shows error rates versus the number of features and, surprisingly, Spearman's R does a good job for all but the bulldozer data set. PCA is the opposite, performing well on bulldozer but poorly on the others. \simp{} is competitive with or surpasses these baseline techniques.  Ordinary least squares (OLS) regression coefficients, divided by their standard error, are included as a common reference curve on this and the following graphs. \todo{25k records, 80/20 split, 40 trees etc..., 10 runs, 5-fold CV}

Next, we compare \simp{} to OLS, OLS-based SHAP, RF-based SHAP, and RF-based permutation importance in \figref{fig:topk}.  We have given SHAP every advantage here.  Except for OLS-based SHAP, SHAP uses a RF fit to the training data to compute feature importances and then trains the same kind of model (RF) to measure top-$k$ error curves.  Further, SHAP selects features using the validation data, the very data used to compute error curves. In contrast, \simp{} cannot use predictions from a fitted model, by design, and we restrict \simp{} to examining just the training data, without access to the validation data. Despite these handicaps, \simp{} error curves for the four data sets are competitive with the  feature rankings from the other methods. OLS does not appear to adequately capture the relationship between features and response variable, except for the Boston data set, which explains why the OLS-based SHAP error curves are higher than for the other methods. (Curiously, features ranked by OLS SHAP perform worse than plain OLS.) \simp{} performs competitively with existing techniques, sometimes a little worse and sometimes a bit better than RF-based SHAP and RF-based permutation importance, but \simp{} does so without the expertise of a fitted model and with access to less data.

We define feature importance as the area under the partial dependence curve weighted by the $x_j$ density, which takes into consideration validation sets with highly skewed distributions.  For the four data sets in use here, we found that the plain impact, area under the partial dependence curve, also works well, as shown in \figref{fig:topk-impact}. In fact, the first feature recommended by \simp{} for the bulldozer data set gets half the error rate  of the top features recommended by the other methods, \figref{fig:topk-impact}(c). \simp{} recommends the {\tt ModelID} followed by {\tt YearMade}, whereas RF-based SHAP selects {\tt ProductSize}, {\tt YearMade}, followed by {\tt ModelID}. \todo{tie into the importance bar charts}

Features that are predictive in one model are not necessarily predictive in another model.  To determine how well the feature rankings from \simp{}, OLS, RF permutation importance, and OLS/RF-based SHAP  ``export'' to other models, we trained gradient boosting machines (GBM) with the previous rankings, as shown in \figref{fig:topk-gbm}. The error curves generated using GBM models are similar, except for the flight delay data set which does not improve after feature five as error curves for the other feature rankings do.  This could be the choice of features or the fact that we did not tune the GBM hyper-parameters per dataset. 

GBMs are based upon decision trees (stumps) like random forests, so we computed error curves for the feature rankings using Lasso regularized regression as well, \figref{fig:lasso}. The rent data set was the only one without categorical variablesThe \simp{} error curve does not drop significantly after the top three features, which could mean that the \simp{} feature rankings require a complex model
...
\subcaption{\footnotesize foo  }
\end{subfigure}
\caption{\small variation appears from run to run given the subset of overall data used for these experiments}
\label{fig:baseline}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.5]{images/boston-topk-Lasso-Importance.pdf}~~~
\includegraphics[scale=0.5]{images/rent-topk-Lasso-Importance.pdf}
\captionof{figure}{\small Lasso model $lambda=1.0$}
\label{fig:lasso}
\end{center}
\end{figure}
