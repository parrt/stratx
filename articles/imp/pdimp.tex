...
\begin{proof}
For independent features, we can choose any subset $S \subseteq F \texttt{\char`\\} \{j\}$ to measure the impact of feature $j$, therefore, we choose $S = F \texttt{\char`\\} \{j\}$. Since ${{p-1}\choose{|S|}} = {{p-1}\choose{p-1}} = 1$, the weight reduces to $1/p$ for all $j$ features:
...
\noindent But, that is equivalent to the approximation of $j$'s partial dependence (Equation 53 in \citealt{PDP}), \todo{use PDP for friedman as differs from our PD} giving:
...
$\phi_j$ is, therefore, the difference between points on two partial dependence curves, one curve derived using $F$ and one derived from $F \texttt{\char`\\} \{j\}$. The $\phi_j$ values across all $x^{(i)}$ constitute a mean-centered partial dependence curve. 

Because $\text{\it PDP}_{F}(x) = \hat{f}_F(x_F) = \hat{f}(x)$, the equation simplifies to the marginal effect of $j$ on $\hat{f}(x)$:

\[
\phi_j(\hat{f},x) = \hat{f}(x) - \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x)
\]

What is expected value of $\phi_j$? Should be 0

\[
\Ex[ \phi_j(\hat{f},x) ] = \Ex [\hat{f}(x)] - \Ex[ \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x) ]
\]

Expand expectations across ${\bf X}$:

\[
\Ex[ \phi_j(\hat{f},x) ] = \frac{1}{n} \sum_{k=1}^n \hat{f}(x^{(k)}) - \
\frac{1}{n} \sum_{k=1}^n \Ex[ \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x) ]
\]

\[
\Ex[ \phi_j(\hat{f},x) ] = \overline{\hat{f}(x)} - \
