...
\todo{Ok, got it from code: ``Since most models aren't designed to handle arbitrary missing data at test
...
Shapley regression values for features $F=\{1,..,p\}$ per \cite{shap} but using model $\hat{f}$ not $f$ and simplifying the weights:
...
The contribution of feature $j$ is the difference between $\hat{f}(x)$ for $x$ with and without $j$, $\hat{f}(x_{S \cup \{j\}}) - \hat{f}(x_S)$. SHAP approximates $\hat{f}(x_S)$ with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}}) | {\bf X}_S = x_S]$ then further approximates with $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ by assuming independent features. \cite{janzing2019feature} argues that $\Ex[\hat{f}(x_{S},{\bf X}_{\overline{S}})]$ is ``... conceptually the right thing in the first place.''

\begin{theorem}
The Shapley regression values for $x^{(i)} \in \bf X$ estimate the mean-centered partial dependence curve as defined by \cite{PDP}, assuming independent features.
\end{theorem}

\begin{proof}
{\em Part 1: } Show $\phi_j(\hat{f},x)$ is the difference between $\hat{f}(x)$ and the partial dependence curve for all features except $j$, as defined by Friedman. For independent features, we can choose any subset $S \subseteq F \texttt{\char`\\} \{j\}$ to measure the impact of feature $j$, therefore, we choose $S = F \texttt{\char`\\} \{j\}$. Since ${{p-1}\choose{|S|}} = {{p-1}\choose{p-1}} = 1$, the weight reduces to $1/p$ for all $j$ features:
...
\noindent Replacing $\hat{f}_S(x_{S})$ by $\Ex[\hat{f}(x_{S},{\bf X}_{\bar{S}})]$ and, because $S$ is fixed, the summation disappears:
...
\noindent But, that summation is equivalent to the approximation of $j$'s partial dependence (Equation 53 in \citealt{PDP}), \todo{use PDP for friedman as differs from our PD} giving:

\[
\phi_j(\hat{f},x) = \frac{1}{p}\
 [  \text{\it PDP}_{S \cup \{j\}}(x) - \text{\it PDP}_{S}(x) ]
\]
...
\[
\phi_j(\hat{f},x) = \frac{1}{p}\
\text{\it PDP}_{F}(x) - \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x)
\]
...
Because $\text{\it PDP}_{F}(x) = \hat{f}_F(x_F) = \hat{f}(x)$, the equation simplifies to the difference between $\hat{f}(x)$ and the marginal effect of $j$ at point $x$:

\[
\phi_j(\hat{f},x) = \hat{f}(x) - \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x)
\]

\noindent {\em Part 2}. Show that SHAP curves are mean-centered PDP curves:

\[
\phi_j(\hat{f},x) = \text{\it PDP}_j(x) - \bar{y}
\]

\noindent Take the expected value of $\phi_j(\hat{f},x)$, as defined in the previous part, across all $x \in X$:

\[
\Ex_{\bf X}[ \phi_j(\hat{f},x) ] = \Ex_{\bf X} [\hat{f}(x) - \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x) ]
\]

\[
\Ex_{\bf X}[ \phi_j(\hat{f},x) ] = \Ex_{\bf X} [\hat{f}(x) ] - \Ex_{\bf X} [ \text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x) ]
\]

\noindent $\Ex_{\bf X} [\hat{f}(x)]$ is just $\overline{\hat{f}({\bf X})} = \bar{y}$ and $\text{\it PDP}_{F \texttt{\char`\\} \{j\}}(x)$ is equivalent to $ICE_j(x)$, the Individual Conditional Expectation ({\em ICE}) from \cite{ice}, which simplifies the equation to: 
